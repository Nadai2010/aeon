tweet drafts: treg — the OpenRouter for agent tool credentials
...
---
- OpenRouter's failure mode: your app calls the wrong model. Treg's is every SaaS credential your team owns.
- The proxy token is identical across all 60+ connected providers — no per-tool scope baked in.
- Stored secrets are Fernet-encrypted, but the encryption key auto-generates at boot if you skip config — a restart can quietly wipe them.
- A third-party catalog already rates it "Critical Risk, not audited."
- Consolidation isn't the same thing as safety. It just moves where the single point of failure lives.
...
---
- The pitch: 3,000+ endpoints, 60+ providers, one X-Treg-Token header, agents never touch a real key.
- The gap: that token is identical across every provider you've connected — not scoped per tool.
- Compare to the model it's clearly modeled after: a leaked OpenRouter key burns your model budget. A leaked treg token can plausibly touch Stripe, scraping, and social APIs in the same session.
- Bonus footgun: secrets are encrypted with a key that auto-generates at boot if you never set TREG_SECRET_KEY — an unplanned restart silently drops your stored credentials.
- Every "unify N services behind one key" project makes this same tradeoff. Most just don't say it out loud.

best: #4a — long tweet / structural critique, most complete argument with the numbers to back it
